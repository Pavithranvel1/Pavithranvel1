...
export default function ConfirmPurchase({ formStep, setData, data }) {
  ...
  const handleSubmit = (values) => {
    ...
    if(Object.keys(formData).length !== 0 && formIsValid) {
      ...
      axios
      .post('https://stagingdev-kap.com/gravityform.php', {"form_data": finaldata})
      .then(response => {
  ...
  const validate = (data) => {
    ...
    fields.map((item, i) => {
      if(!data[item.name] && !(fundspecialist == false && item.name == 'phone')){
        formIsValid = false;
    ...
    {showMe && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
        <div className={`${error2?.first_name ? `cfield-error` : ``}`}>
          <input name="first_name" placeholder="First Name" className="bg-kapitus border-2 border-gray-300 py-3 w-full text-lg text-white font-semibold mb-4 p-2 focus:outline-none" type="text" onChange={event => handleChange(event)} onBlur={event => handleBlur(event)} defaultValue={formData?.first_name} />
          {error2?.first_name && <span className="text-formred text-xl">{error2.first_name}</span>}
        </div>
        <div className={`${error2?.last_name ? `cfield-error` : ``}`}>
          <input name="last_name" placeholder="Last Name" className="bg-kapitus border-2 border-gray-300 py-3 w-full text-lg text-white font-semibold mb-4 p-2 focus:outline-none" type="text" onChange={event => handleChange(event)} onBlur={event => handleBlur(event)} defaultValue={formData?.last_name} />
          {error2?.last_name && <span className="text-formred text-xl">{error2.last_name}</span>}
        </div>
        <div className={`${error2?.company_name ? `cfield-error` : ``}`}>
          <input name="company_name" placeholder="Company Name*" className="bg-kapitus border-2 border-gray-300 py-3 w-full text-lg text-white font-semibold mb-4 p-2 focus:outline-none" type="text" onChange={event => handleChange(event)} onBlur={event => handleBlur(event)} defaultValue={formData?.company_name} />
          {error2?.company_name && <span className="text-formred text-xl">{error2.company_name}</span>}
        </div>
        <div className={`${error2?.email ? `cfield-error` : ``}`}>
          <input name="email" placeholder="Please provide your email address to see your results:*" className="bg-kapitus border-2 border-gray-300 py-3 w-full text-lg text-white font-semibold mb-4 p-2 focus:outline-none" type="text" onChange={event => handleChange(event)} onBlur={event => handleBlur(event)} defaultValue={formData?.email} />
          {error2?.email && <span className="text-formred text-xl">{error2.email}</span>}
        </div>
      </div>
      <div className="grid grid-cols-1">
        <label className="py-6 text-xl font-bold gfield_label text-white">Web Site URL</label>
        <input name="website_url" placeholder="https://example.com" className="bg-kapitus border-2 border-gray-300 py-3 w-full text-lg text-white font-semibold mb-4 p-2 focus:outline-none" type="text" onBlur={event => handleBlur(event)} defaultValue={formData?.website_url} />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
        <div className={`${fundspecialist ? `block` : `hidden` } ${error2?.phone ? ` cfield-error` : ``}`}>
          <input name="phone" placeholder="Please provide the best number to reach you:" className="bg-kapitus border-2 border-gray-300 py-3 w-full text-lg text-white font-semibold mb-4 p-2 focus:outline-none" type="text" onChange={event => handleChange(event)} onBlur={event => handleBlur(event)} value={phone} />
          {error2?.phone && <span className="text-formred text-xl">{error2.phone}</span>}
        </div>
        <div className={`${error2?.about_us ? `cfield-error` : ``}`}>
          <select name="about_us" className="block w-full px-4 py-3 bg-kapitus text-white border border-gray-300 mb-4 focus:outline-none" onChange={event => handleChange(event)} onBlur={event => handleBlur(event)} value={formData?.about_us}>
          <option value="">How did you hear about us?</option>
          {aboutus.map((option, i) =>
          <option value={option} key={i}>{option}</option>)}
          </select>
          {error2?.about_us && <span className="text-formred text-xl pt-3">{error2.about_us}</span>}
        </div>
      </div>
        <div className="flex">
        <div className="w-1/2">
          <button className="text-kapitus bg-white rounded p-1 my-4 mt-6 py-2 px-7" onClick={handlePrev} type="button">Back</button>
        </div>
