...
export default function Industry({ formStep, setData,  data }) {
  ...
  const [formData, setFormData] = useState( gfindustry ? { ['industry'] : gfindustry } : []);
          ...
          {industries.map((industry, i) =>
            <span className="py-3" key={i}>
              <input name="industry" type="radio" defaultChecked={industry.value == gfindustry ? `checked`: (!industries.find(item => item.value.includes(gfindustry)) && gfindustry !==`` ? `checked`:``)} id={industry.id} value={industry.value} onClick={handleClick} />
                {industry.id !== 'others' ? (<label htmlFor={industry.id} className={`ml-6 ${error ? `text-formred` : `text-white`}`}>{industry.value}</label>): (<input type="text" className="ml-6 bg-kapitus border-b-2 border-white w-2/5 text-white py-2 focus:outline-none" name="industry" id={`${industry.id}_text`} placeholder='Others' defaultValue={!industries.find(item => item.value.includes(gfindustry)) ? gfindustry: ``} onClick={handleClick} onBlur={handleBlur} /> )}
            </span>
          )}
