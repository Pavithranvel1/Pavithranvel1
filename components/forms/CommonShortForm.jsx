...
import { equipIndustries, lists, creditScores, aboutUs, months, days, years, states, contact, shortForm } from '../Variables'
...
function CommonShortForm({
  ...
  const onSubmit = (data) => {
    ...
    requestForm[31] = window.location.href //full path embed url
      ...
      .then(response => {
        if(response.data) {
          ...
          if(data.sales == 'complete the full application'){
            ...
            router.push(`/fast-application?${res}`)
          }
          else {
...
/*       axios
  ...
  return (
      <section className="w-full max-w-4xl mt-2 px-8 m-auto shortForm">
      ...
      <div className="col-span-2 text-left mb-4">
        ...
        <input className="py-3 px-6 bg-kapitusLiteGreen text-white font-bold w-full" type="submit" />
      </div>
        </div>
      </form>
      </section>
