...
const Commonshortform = ({ refill, credentials }) => {
  ...
  let preloadedValues = {
    business_name: refill?.business_name,
    use_fund: refill?.use_fund,
    first_name: refill?.first_name,
    last_name: refill?.last_name,
    email_address: refill?.email_address,
    industry: refill?.industry,
    month: refill?.month,
    day: refill?.day,
    year: refill?.year,
    invoice: refill?.invoice,
    invoice_worth: refill?.invoice_worth,
    sales: refill?.sales,
  };
  ...
  useEffect(() => {
    ...
    router.query?.gf_token,
    refill?.phone_number,
    refill?.fund,
    refill?.annual_revenue,
  ]);
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="col-span-2 sm:col-span-1 md:col-span-1 relative">
            <input
              ...
              className="w-full p-2 mt-2 text-sm bg-white form-border border-gray-300 text-kapitus placeholder-liteblue focus:outline-none"
              {...register("fund", { required: fund ? false : true })}
          ...
          <div className="col-span-2 sm:col-span-1 md:col-span-1 relative">
            ...
            {watch("business_name") &&
              typeof errors.business_name?.type == "undefined" && (
                <span className="success"></span>
              )}
          </div>
          <div className="col-span-2 sm:col-span-1 md:col-span-1 relative">
            <select
              className="w-full p-2 mt-2 text-sm bg-white form-border border-gray-300 text-kapitus"
            ...
            {watch("industry") &&
              typeof errors.industry?.type == "undefined" && (
                <span className="success"></span>
              )}
          </div>
          <div className="col-span-2 sm:col-span-1 md:col-span-1 relative">
            ...
            {watch("use_fund") &&
              typeof errors.use_fund?.type == "undefined" && (
                <span className="success"></span>
              )}
          </div>
          <div className="col-span-2 sm:col-span-1 md:col-span-1 relative">
          ...
          <div className="col-span-2 sm:col-span-1 md:col-span-1 relative">
            <input
              ...
              {...register("phone_number", {
                required: /^(\()?\d{3}(\))?(-|\s)?\d{3}(-|\s)\d{4}$/.test(phone)
                  ? false
                  : true,
              })}
              //onBlur={(event) => handleBlur(event)}
          ...
          <div className="col-span-2 sm:col-span-1 md:col-span-1 relative">
            ...
            {watch("credit_score") &&
              typeof errors.credit_score?.type == "undefined" && (
                <span className="success"></span>
              )}
          </div>
          <div className="col-span-2 sm:col-span-1 md:col-span-1 relative">
          ...
          <div className="col-span-2 sm:col-span-1 md:col-span-1 relative">
            ...
            >
              ...
              {aboutUs.map((about_us, i) => (
                <option value={about_us} key={i}>
                  {about_us}
                </option>
              ))}
            </select>
            {errors.about_us?.type === "required" && (
              <span className="text-errorred m-2">
            ...
            {watch("about_us") &&
              typeof errors.about_us?.type == "undefined" && (
                <span className="success"></span>
              )}
          </div>
          <div className="col-span-2 sm:col-span-1 md:col-span-1 py-2 text-sm text-kapitus w-full gap-1 relative">
            <div className="flex">
              <select
                className="border border-gray-200 p-2 rounded w-1/3"
              ...
              >
                ...
                {months.map((option, i) => (
                  <option value={option} key={`${i}month`}>
                    {option}
                  </option>
                ))}
              </select>
              ...
              >
                ...
                {years.map((option, i) => (
                  <option value={option} key={`${i}year`}>
                    {option}
                  </option>
                ))}
              </select>
            ...
            {watch("month") &&
              watch("day") &&
              watch("year") &&
              typeof errors.month?.type == "undefined" &&
              typeof errors.day?.type == "undefined" &&
              typeof errors.year?.type == "undefined" && (
                <span className="success" />
              )}
          </div>
          <div className="col-span-2 relative">
            <span className="flex">
              <Image
                ...
                className="bg-center bg-contain inline-block mr-6 h-10 w-10 bg-no-repeat"
              />
              <label className="font-bold text-kapitus text-sm ml-5">
                I would like to
              </label>
            </span>
            <select
              className="w-full px-4 py-2 mt-2 text-sm bg-white form-border border-gray-300 text-kapitus relative"
              defaultValue={sales}
              onChange={(event) => handleChange(event)}
            ...
            {errors.sales?.type === "required" && (
              <span className="text-errorred m-2">
                This Field is Required
                <span className="error" />
              </span>
            )}
            {watch("sales") && typeof errors.sales?.type == "undefined" && (
              <span className="success" />
            )}
          </div>
          <div className="col-span-2 text-kapitus text-xs">
            Fees may apply. Kapitus needs the contact information you provide to
