...
import { lists, states, months, days, years, aboutUs, entities, findustries, creditScores, percentage, formData  } from '../Variables';
...
/*     axios
    .post('https://ada-kapitus.com/next-js/applynow.php', {"formData": requestForm, 'fileData': multipleFiles, 'signature': signature}, config)
    .then(response => {
  ...
  return (
      <div className="max-w-4xl m-auto bg-white p-8">
        ...
        <div className="grid grid-cols-2 gap-4 max-w-4xl m-auto">
          ...
          <div className="col-span-2 md:col-span-1">
            <select className="border-solid border-gray-300 border-2 h-12 p-2 text-base w-full text-liteblue font-semibold  placeholder-liteblue text-base w-full md:w-34" {...register("use_fund", { required: true })}>
          ...
          <div className="col-span-2 md:col-span-1">
            <input type="text" className="border-solid border-gray-300 border-2 h-12 p-2 text-liteblue font-semibold  placeholder-liteblue  text-lg w-full" {...register("business_name", { required: true })} placeholder="Legal Business Name" onBlur={handleBlur} />
            {errors.business_name?.type === 'required' && (<span className="text-errorred m-2">Legal Business Name is required</span>)}
          </div>
          <div className="col-span-2 md:col-span-1">
          ...
          <div className="col-span-2 md:col-span-1 flex space-x-3 w-full">
            ...
            <select className="border-solid border-gray-300 border-2 h-12 p-2 text-liteblue font-semibold  placeholder-liteblue  text-base w-full md:w-34" {...register("business_state", { required: true })}>
          ...
          {multipleFiles.map((item, index)=>
            <span className="col-span-2 h-5 flex" key={index} onClick={event => handleClick(event)}>
              <Image alt="delete" width="20" height="5" className="mr-3" id={index.toString()} src="/images/delete.png" /><label className="ml-3 -mt-1 text-lg text-kapitus font-bold" htmlFor={index.toString()}>{item.name}</label>
            </span>
          )}
          ...
          <div className="col-span-2 md:col-span-1">
            ...
            <button type="button" className="col-span-2 w-72 sm:w-full bg-white p-0 mt-0 relative refresh cursor-pointer" onClick={event => handleClear(event, padRef)}><Image alt="refresh" width="30" height="30" src="/images/refresh.png" {...commonname} className="right bg-contain inline-block h-8 w-8 bg-no-repeat" /></button>
          ...
          <div className={`col-span-2 md:col-span-1 ${percent==100 ? `h-0 overflow-hidden` : `block`}`}>
            ...
            <SignatureCanvas penColor='green' clearButton="true" ref={padRef1} canvasProps={{ className: 'col-span-2 h-44 w-72 sm:w-full border-2 border-gray-500 border-dashed bg-white mt-5', id: 72}} onEnd={event => handleChange(event, padRef1)} />
              <button type="button" className="col-span-2 w-72 sm:w-full bg-white p-0 mt-0 relative refresh" onClick={event => handleClear(event, padRef1)}><Image alt="refresh" width="30" height="30" src="/images/refresh.png" {...owner2name} className="right bg-contain inline-block h-8 w-8 bg-no-repeat" /></button>
